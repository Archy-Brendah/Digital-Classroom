...
def Reply(request):
    print(request.POST)
    if request.method == 'POST':
        mess1 = request.POST['mess_object']
        mess2 = Message.objects.get(id=mess1)
        message = Message(Party=mess2.sender, sender=request.user, Title=mess2.Title, Content=request.POST['content'])
        message.save()
...
def my_message(request):
    ...
    if request.method == 'POST':
        ...
        print(request.POST)
        if form.is_valid():
            user = User.objects.get(username=request.POST['party'])
            message = Message(Party=user, sender=request.user, Title=request.POST['Title'], Content=request.POST['Content'])
            message.save()
            form = MessForm()
    ...
    return render(request, 'students/Message.html', {'form': form})
